...
| [`dots-studio/dots.tts-mf`](https://huggingface.co/dots-studio/dots.tts-mf)<br>[`dots-studio/dots.tts-mf-2steps`](https://huggingface.co/dots-studio/dots.tts-mf-2steps)<br>[`dots-studio/dots.tts-mf-1step`](https://huggingface.co/dots-studio/dots.tts-mf-1step) | `dots.tts` / `DotsTtsRuntime` | Latency- or concurrency-sensitive TTS. | `mf`: NFE `4` recommended; CFG fused.<br>`mf-2steps`: omit sampling options; fixed NFE `2` sCM.<br>`mf-1step`: omit sampling options; fixed NFE `1`. | MeanFlow checkpoints distilled from `dots.tts-soar`, with different quality / inference-cost tradeoffs. |
